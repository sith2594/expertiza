...
class AccountRequestController < ApplicationController
  ...
  def create_approved_user
    ...
    users.each do |user|
      ...
      requested_user.status = params[:commit] #Using params[:commit] instead of params[:status]
  ...
  def list_pending_requested
    #@requested_users = AccountRequest.where('status = ?', 'Under Review') #display only pending requests
    # @requested_users = AccountRequest.all.order(created_at: :desc) #display all in reverse chrolonogical order
    @requested_users = AccountRequest.all.order(created_at: :desc).paginate(page: params[:page], per_page: 10) #display all requests with pagination and in reverse chronologigal order
